...
void GnssHeading::send(const uint64_t time)
{
	...
	const float dt = static_cast<float>(time - _gnss_heading_data.time_us) * 1e-6f;
	(void)dt;

	_gnss_heading_data.time_us = time;

	// hand the sample to the EKF
	_ekf->setGnssHeadingData(_gnss_heading_data);
}

void GnssHeading::setData(const gnssHeadingSample &gps_yaw)
{
...
gnssHeadingSample GnssHeading::getDefaultGnssHeadingData()
{
	gnssHeadingSample gps_yaw{};
	gps_yaw.time_us = 0;
	gps_yaw.heading = NAN;
